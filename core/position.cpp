...
void Position::set(const std::string& fen, Position& p) {
	...
	case '2':
		...
		break;
	case '3':
		epsq = Square(epsq0 + 16);
		break;
	case '4':
		epsq = Square(epsq0 + 24);
		break;
	case '5':
		epsq = Square(epsq0 + 32);
		break;
	case '6':
		epsq = Square(epsq0 + 40);
		break;
	case '7':
		epsq = Square(epsq0 + 48);
		break;
	case '8':
		epsq = Square(epsq0 + 56);
		break;
	}
	idx++; // skip space
	}
	...
	p.history[p.game_ply].halfmove_clock = std::stoi(token_str);
	token_str = "";
	idx++; // skip space
	while (idx < size && flags[idx] != ' ') {
		token_str += flags[idx++];
